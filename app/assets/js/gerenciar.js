...
document.getElementById("formImovel").addEventListener("submit", async function (e) {
    ...
    try {
        ...
        if (data.sucesso) {
            ...
            await carregarAreas(); // Recarregar áreas pois o select pode ter mudado
            atualizarResumo();
        }
    } catch (error) {
    ...
    try {
        ...
        areas.forEach(area => {
            ...
            tr.innerHTML = `
                ...
                <td>
                    <div class="action-buttons">
                        <button onclick="editarArea(${area.id}, '${area.nome}', ${area.imovel_id})" class="btn btn-sm btn-secondary" title="Editar">
                            <i class="fas fa-edit"></i>
                        </button>
...
document.getElementById("formArea").addEventListener("submit", async function (e) {
    ...
    try {
        ...
        if (data.sucesso) {
            resetForm("formArea");
            await carregarAreas();
            await carregarEquipamentos(); // Recarregar equipamentos pois o select pode ter mudado
            atualizarResumo();
        }
    } catch (error) {
...
async function carregarEquipamentos() {
    try {
        ...
        equipamentos.forEach(equip => {
            const consumoDiario = (equip.potencia * equip.horas_por_dia) / 1000;
            ...
            tr.innerHTML = `
                ...
                <td>
                    <div class="action-buttons">
                        <button onclick="editarEquipamento(${equip.id}, '${equip.nome}', '${equip.modelo || ''}', ${equip.potencia}, ${equip.horas_por_dia}, ${equip.area_id})" class="btn btn-sm btn-secondary" title="Editar">
                            <i class="fas fa-edit"></i>
                        </button>
